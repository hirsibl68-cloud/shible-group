...
// نخلي الصفحة ديناميكية عشان ما يحاول Next يـ pre-render
...
export default function DepositPage() {
  ...
  function submit() {
    if (amount < MIN)
      return setMsg({ type: "err", text: `الحد الأدنى ${MIN}` });
    if (amount > MAX)
      return setMsg({ type: "err", text: `الحد الأقصى ${MAX}` });
    if (!agree) return setMsg({ type: "err", text: "وافق على الشروط" });
    if (amount > balance)
      return setMsg({ type: "err", text: "رصيد غير كافٍ" });
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="rounded-2xl border border-[#dce9ff] bg-white p-5 shadow-sm space-y-5">
          ...
          <div className="space-y-1">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="rounded-xl border border-[#dce9ff] bg-white px-3 py-2 text-sm hover:bg-[#f3f9ff]"
              >
                500
              </button>
              <button
              ...
              {PLANS.map((p) => (
                <button
                  ...
                  className={`text-left rounded-2xl border p-4 transition w-full ${
                    planId === p.id
                      ? "border-[#2aa1ff] bg-[#e9f5ff]"
                      : "border-[#dce9ff] bg-white hover:bg-[#f6fbff]"
                  }`}
                >
          ...
          <div className="grid md:grid-cols-3 gap-3">
            ...
            <Info
              label={`عائد ${plan.lockDays} يوم`}
              value={`${expectedTotal} ${currency}`}
            />
            <Info
              label="نسبة يومية"
              value={`${(plan.rateDaily * 100).toFixed(1)}%`}
            />
          </div>
          ...
          <label className="flex items-start gap-2">
            <input
              ...
              className="mt-1 accent-[#2aa1ff]"
            />
            <span className="text-sm text-[#345067]">
              أوافق على سياسة الإيداع والأرباح الثابتة
            </span>
          </label>
          ...
          <button
            onClick={submit}
            className="w-full rounded-xl py-3 font-semibold bg-[#2aa1ff] hover:bg-[#1b8aea] text-white"
          >
            تأكيد الإيداع
          </button>
        </div>
